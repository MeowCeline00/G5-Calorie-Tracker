document.addEventListener('DOMContentLoaded', function() {
  ...
  const updateCalButton = () => {
    const totalCaloriesFromMeals = Array.from(document.querySelectorAll('.number'))
        .map(item => parseInt(item.textContent))
        .reduce((acc, curr) => acc + curr, 0);
    calButton.textContent = `${totalCaloriesFromMeals} cal`;

    // Display total calories in the HTML file
    const iframe = document.getElementById('externalFrame');
    const iframeDocument = iframe.contentDocument || iframe.contentWindow.document;
    const cal1Span = iframeDocument.querySelector('.cal1 span');
    if (cal1Span) {
      cal1Span.textContent = totalCaloriesFromMeals;
    }
  };
